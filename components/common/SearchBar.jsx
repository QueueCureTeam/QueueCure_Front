...
export default function SearchBar({ showDropdown, onStatusChange, onSearchChange }) {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-4">
      <div className="flex w-full rounded-xl shadow-md border border-blue-200 bg-white relative">
            ...
            {open && (
              <div className="absolute top-full left-0 mt-1 w-40 bg-white border border-gray-300 rounded-lg shadow-xl py-1 z-20">
                {statusOptions.map((option) => (
                  <button key={option.value} type="button" onClick={() => handleSelect(option)} className={`block w-full text-left px-4 py-2 text-sm ${selected.value === option.value
        ...
        <input type="text" placeholder="ค้นหา" value={inputValue} onChange={getInputValue} onKeyDown={handleEnterKey} className="flex-1 px-4 py-3 text-sm text-gray-700 placeholder-gray-400 focus:outline-none border-none focus:ring-0" />
        <button type="button" onClick={sendInputValue} className="px-6 py-3 bg-blue-500 text-white flex items-center justify-center rounded-r-xl hover:bg-blue-600 transition-all duration-200">
          <FaSearch size={16} />
        </button>
      </div>
    </div>
